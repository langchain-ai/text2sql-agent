...
from agents.simple_text2sql import generate_sql, execute_sql, generate_answer
...
def test_generate_sql_node():
    ...
    state = {"messages": [HumanMessage(content="List all artists")], "schema": "", "sql": "", "records": []}
    result = node(state)
...
def test_execute_sql_node():
    ...
    state = {"sql": "SELECT * FROM artists", "messages": [], "schema": "", "records": []}
    result = node(state)
